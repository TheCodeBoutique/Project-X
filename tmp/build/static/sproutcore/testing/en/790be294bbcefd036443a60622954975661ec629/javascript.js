...
CoreTest={K:function(){return this},beget:function(obj){if(!obj){return null}var K=CoreTest.K;
K.prototype=obj;var ret=new K();K.prototype=null;return ret},mixin:function(){var target=arguments[0]||{};
var idx=1;var length=arguments.length;var options;if(length===1){target=this||{};
idx=0}for(;idx<length;idx++){if(!(options=arguments[idx])){continue}for(var key in options){if(!options.hasOwnProperty(key)){continue
}var src=target[key];var copy=options[key];if(target===copy){continue}if(copy!==undefined){target[key]=copy
}}}return target},fmt:function(str){var args=arguments;var idx=1;return str.replace(/%@([0-9]+)?/g,function(s,argIndex){argIndex=(argIndex)?parseInt(argIndex,0):idx++;
s=args[argIndex];return((s===null)?"(null)":(s===undefined)?"":s).toString()})},stub:function(name,func){var attrs={};
if(typeof func==="function"){attrs.action=func}else{if(typeof func==="object"){attrs=func
}}var ret=function(){ret.callCount++;var args=[],loc=arguments.length;while(--loc>=0){args[loc]=arguments[loc]
}args.unshift(this);ret.history.push(args);return ret.action.apply(this,arguments)
};ret.callCount=0;ret.history=[];ret.stubName=name;var key;for(key in attrs){if(!attrs.hasOwnProperty(key)){continue
}ret[key]=attrs[key]}if(!ret.reset){ret.reset=function(){this.callCount=0;this.history=[]
}}if(!ret.action){ret.action=function(){return this}}if(!ret.expect){ret.expect=function(callCount){if(callCount===YES){ok(this.callCount>0,CoreTest.fmt("%@ should be called at least once",this.stubName))
}else{if(callCount===NO){callCount=0}equals(this.callCount,callCount,CoreTest.fmt("%@ should be called X times",this.stubName))
}}}return ret},OK:"passed",FAIL:"failed",ERROR:"errors",WARN:"warnings",showUI:false};
if(!String.prototype.camelize){String.prototype.camelize=function camelize(){var ret=this.replace(SC.STRING_TITLEIZE_REGEXP,function(str,separater,character){return(character)?character.toUpperCase():""
...
}}if(!String.prototype.fmt){String.prototype.fmt=function fmt(){var args=arguments;
var idx=0;return this.replace(/%@([0-9]+)?/g,function(s,argIndex){argIndex=(argIndex)?parseInt(argIndex,0)-1:idx++;
s=args[argIndex];return((s===null)?"(null)":(s===undefined)?"":s).toString()})}}if(!Array.prototype.uniq){Array.prototype.uniq=function uniq(){var ret=[],len=this.length,item,idx;
for(idx=0;idx<len;idx++){item=this[idx];if(ret.indexOf(item)<0){ret.push(item)}}return ret
}}if(!String.prototype.w){String.prototype.w=function w(){var ary=[],ary2=this.split(" "),len=ary2.length;
for(var idx=0;idx<len;++idx){var str=ary2[idx];if(str.length!==0){ary.push(str)}}return ary
}}if(typeof CHANCE_SLICES==="undefined"){var CHANCE_SLICES=[]}CHANCE_SLICES=CHANCE_SLICES.concat([]);
function notest(name,callback,nowait){}(function(){
/*
 *
...
}})})();var Q$=jQuery.noConflict(true);sc_require("jquery");(function($){var equiv=function(){var innerEquiv;
var callers=[];function hoozit(o){if(typeof o==="string"){return"string"}else{if(typeof o==="boolean"){return"boolean"
}else{if(typeof o==="number"){if(isNaN(o)){return"nan"}else{return"number"}}else{if(typeof o==="undefined"){return"undefined"
}else{if(o===null){return"null"}else{if(o instanceof Array){return"array"}else{if(o instanceof Date){return"date"
}else{if(o instanceof RegExp){return"regexp"}else{if(typeof o==="object"){return"object"
}else{if(o instanceof Function){return"function"}}}}}}}}}}}function bindCallbacks(o,callbacks,args){var prop=hoozit(o);
if(prop){if(hoozit(callbacks[prop])==="function"){return callbacks[prop].apply(callbacks,args)
}else{return callbacks[prop]}}}var callbacks=function(){function useStrictEquality(b,a){return a===b
}return{string:useStrictEquality,"boolean":useStrictEquality,number:useStrictEquality,"null":useStrictEquality,"undefined":useStrictEquality,nan:function(b){return isNaN(b)
},date:function(b,a){return hoozit(b)==="date"&&a.valueOf()===b.valueOf()},regexp:function(b,a){return hoozit(b)==="regexp"&&a.source===b.source&&a.global===b.global&&a.ignoreCase===b.ignoreCase&&a.multiline===b.multiline
},"function":function(){var caller=callers[callers.length-1];return caller!==Object&&typeof caller!=="undefined"
},array:function(b,a){var i;var len;if(!(hoozit(b)==="array")){return false}len=a.length;
if(len!==b.length){return false}for(i=0;i<len;i++){if(!innerEquiv(a[i],b[i])){return false
}}return true},object:function(b,a){var i;var eq=true;var aProperties=[],bProperties=[];
if(a.constructor!==b.constructor){return false}callers.push(a.constructor);for(i in a){aProperties.push(i);
if(!innerEquiv(a[i],b[i])){eq=false}}callers.pop();for(i in b){bProperties.push(i)
}return eq&&innerEquiv(aProperties.sort(),bProperties.sort())}}}();innerEquiv=function(){var args=Array.prototype.slice.apply(arguments);
if(args.length<2){return true}return(function(a,b){if(a===b){return true}else{if(typeof a!==typeof b||a===null||b===null||typeof a==="undefined"||typeof b==="undefined"){return false
}else{return bindCallbacks(a,callbacks,[b,a])}}})(args[0],args[1])&&arguments.callee.apply(this,args.splice(1,args.length-1))
};return innerEquiv}();var GETParams=$.map(location.search.slice(1).split("&"),decodeURIComponent),ngindex=$.inArray("noglobals",GETParams),noglobals=ngindex!==-1;
if(noglobals){GETParams.splice(ngindex,1)}var config={stats:{all:0,bad:0},queue:[],blocking:true,filters:GETParams,isLocal:!!(window.location.protocol=="file:")};
$.extend(window,{test:test,module:module,expect:expect,ok:ok,equals:equals,start:start,stop:stop,reset:reset,synchronize:synchronize,isLocal:config.isLocal,same:function(a,b,message){push(equiv(a,b),a,b,message)
},QUnit:{equiv:equiv},isSet:isSet,isObj:isObj,compare:function(){throw"compare is deprecated - use same() instead"
...
},q:q,t:t,url:url,triggerEvent:triggerEvent});$(window).load(function(){});function synchronize(callback){config.queue.push(callback);
if(!config.blocking){process()}}function process(){window.config=config;while(config.queue.length&&!config.blocking){config.queue.shift()()
}}function stop(timeout){config.blocking=true;if(timeout){config.timeout=setTimeout(function(){ok(false,"Test timed out");
start()},timeout)}}function start(){setTimeout(function(){if(config.timeout){clearTimeout(config.timeout)
}config.blocking=false;process()},13)}function validTest(name){var i=config.filters.length,run=false;
if(!i){return true}while(i--){var filter=config.filters[i],not=filter.charAt(0)=="!";
if(not){filter=filter.slice(1)}if(name.indexOf(filter)!=-1){return !not}if(not){run=true
}}return run}function runTest(){config.blocking=false;var started=+new Date;config.ajaxSettings=$.ajaxSettings;
synchronize(function(){$('<p id="testresult" class="result"/>').html(["Tests completed in ",+new Date-started," milliseconds.<br/>",'<span class="bad">',config.stats.bad,'</span> tests of <span class="all">',config.stats.all,"</span> failed."].join("")).appendTo("#qunit");
$("#banner").addClass(config.stats.bad?"fail":"pass")})}var pollution;function saveGlobal(){pollution=[];
if(noglobals){for(var key in window){pollution.push(key)}}}function checkPollution(name){var old=pollution;
saveGlobal();if(pollution.length>old.length){ok(false,"Introduced global variable(s): "+diff(old,pollution).join(", "));
config.expected++}}function diff(clean,dirty){return $.grep(dirty,function(name){return $.inArray(name,clean)==-1
})}function test(name,callback){if(config.currentModule){name=config.currentModule+" module: "+name
}var lifecycle=$.extend({setup:function(){},teardown:function(){}},config.moduleLifecycle);
if(!validTest(name)){return}synchronize(function(){var mainEl=document.getElementById("main");
config.fixture=mainEl?mainEl.innerHTML:"";mainEl=null;config.assertions=[];config.expected=null;
try{if(!pollution){saveGlobal()}lifecycle.setup()}catch(e){config.assertions.push({result:false,message:"Setup failed on "+name+": "+e.message})
}});synchronize(function(){try{callback()}catch(e){if(typeof console!="undefined"&&console.error&&console.warn){console.error("Test "+name+" died, exception and test follows");
console.error(e);console.warn(callback.toString())}config.assertions.push({result:false,message:"Died on test #"+(config.assertions.length+1)+": "+e.message});
saveGlobal()}});synchronize(function(){try{checkPollution();lifecycle.teardown()}catch(e){config.assertions.push({result:false,message:"Teardown failed on "+name+": "+e.message})
}});synchronize(function(){try{reset()}catch(e){if(typeof console!="undefined"&&console.error&&console.warn){console.error("reset() failed, following Test "+name+", exception and reset fn follows");
console.error(e);console.warn(reset.toString())}}if(config.expected&&config.expected!=config.assertions.length){config.assertions.push({result:false,message:"Expected "+config.expected+" assertions, but "+config.assertions.length+" were run"})
}var good=0,bad=0;var ol=$("<ol/>").hide();config.stats.all+=config.assertions.length;
for(var i=0;i<config.assertions.length;i++){var assertion=config.assertions[i];$("<li/>").addClass(assertion.result?"pass":"fail").text(assertion.message||"(no message)").appendTo(ol);
assertion.result?good++:bad++}config.stats.bad+=bad;var b=$("<strong/>").html(name+" <b style='color:black;'>(<b class='fail'>"+bad+"</b>, <b class='pass'>"+good+"</b>, "+config.assertions.length+")</b>").click(function(){$(this).next().toggle()
}).dblclick(function(event){var target=$(event.target).filter("strong").clone();if(target.length){target.children().remove();
location.href=location.href.match(/^(.+?)(\?.*)?$/)[1]+"?"+encodeURIComponent($.trim(target.text()))
}});$("<li/>").addClass(bad?"fail":"pass").append(b).append(ol).appendTo("#tests");
if(bad){$("#filter").attr("disabled",null)}})}function module(name,lifecycle){config.currentModule=name;
config.moduleLifecycle=lifecycle}function expect(asserts){config.expected=asserts
}function reset(){$("#main").html(config.fixture);$.event.global={};$.ajaxSettings=$.extend({},config.ajaxSettings)
}function ok(a,msg){config.assertions.push({result:!!a,message:msg})}function isSet(a,b,msg){function serialArray(a){var r=[];
if(a&&a.length){for(var i=0;i<a.length;i++){var str=a[i].nodeName;if(str){str=str.toLowerCase();
if(a[i].id){str+="#"+a[i].id}}else{str=a[i]}r.push(str)}}return"[ "+r.join(", ")+" ]"
}var ret=true;if(a&&b&&a.length!=undefined&&a.length==b.length){for(var i=0;i<a.length;
i++){if(a[i]!=b[i]){ret=false}}}else{ret=false}window.ok(ret,a,b,msg)}function isObj(a,b,msg){var ret=true;
if(a&&b){for(var i in a){if(a[i]!=b[i]){ret=false}}for(i in b){if(a[i]!=b[i]){ret=false
}}}else{ret=false}config.assertions.push({result:ret,message:msg})}function q(){var r=[];
for(var i=0;i<arguments.length;i++){r.push(document.getElementById(arguments[i]))
}return r}function t(a,b,c){var f=$(b);var s="";for(var i=0;i<f.length;i++){s+=(s&&",")+'"'+f[i].id+'"'
}var a1=a,b1=b;var ret=true;a=f;b=q.apply(q,c);if(a&&b&&a.length!=undefined&&a.length==b.length){for(var i=0;
i<a.length;i++){if(a[i]!=b[i]){ret=false}}}else{ret=false}window.ok(ret,a,b,a1+" ("+b1+")")
}function url(value){return value+(/\?/.test(value)?"&":"?")+new Date().getTime()+""+parseInt(Math.random()*100000)
}function equals(actual,expected,message){push(expected==actual,actual,expected,message)
}function push(result,actual,expected,message){message=message||(result?"okay":"failed");
config.assertions.push({result:result,message:result?message+": "+jsDump.parse(expected):message+", expected: "+jsDump.parse(expected)+" result: "+jsDump.parse(actual)})
}function triggerEvent(elem,type,event){if($.browser.mozilla||$.browser.opera){event=document.createEvent("MouseEvents");
event.initMouseEvent(type,true,true,elem.ownerDocument.defaultView,0,0,0,0,0,false,false,false,false,0,null);
elem.dispatchEvent(event)}else{if($.browser.msie){elem.fireEvent("on"+type)}}}})(Q$);
(function(){function quote(str){return'"'+str.toString().replace(/"/g,'\\"')+'"'}function literal(o){return o+""
}function join(pre,arr,post){var s=jsDump.separator(),base=jsDump.indent();inner=jsDump.indent(1);
if(arr.join){arr=arr.join(","+s+inner)}if(!arr){return pre+post}return[pre,inner+arr,base+post].join(s)
}function array(arr){var i=arr.length,ret=Array(i);this.up();while(i--){ret[i]=this.parse(arr[i])
}this.down();return join("[",ret,"]")}var reName=/^function (\w+)/;var jsDump=window.jsDump={parse:function(obj,type){var parser=this.parsers[type||this.typeOf(obj)];
type=typeof parser;return type=="function"?parser.call(this,obj):type=="string"?parser:this.parsers.error
},typeOf:function(obj){var type=typeof obj,f="function";if(obj&&(obj.isObject||obj.isClass)){return"scobj"
}return type!="object"&&type!=f?type:!obj?"null":obj.exec?"regexp":obj.getHours?"date":obj.scrollBy?"window":obj.nodeName=="#document"?"document":obj.nodeName?"node":obj.item?"nodelist":obj.callee?"arguments":obj.call||obj.constructor!=Array&&(obj+"").indexOf(f)!=-1?f:"length" in obj?"array":type
},separator:function(){return this.multiline?this.HTML?"<br />":"\n":this.HTML?"&nbsp;":" "
},indent:function(extra){if(!this.multiline){return""}var chr=this.indentChar;if(this.HTML){chr=chr.replace(/\t/g,"   ").replace(/ /g,"&nbsp;")
}return Array(this._depth_+(extra||0)).join(chr)},up:function(a){this._depth_+=a||1
},down:function(a){this._depth_-=a||1},setParser:function(name,parser){this.parsers[name]=parser
},quote:quote,literal:literal,join:join,_depth_:1,parsers:{window:"[Window]",document:"[Document]",error:"[ERROR]",unknown:"[Unknown]","null":"null",undefined:"undefined","function":function(fn){var ret="function",name="name" in fn?fn.name:(reName.exec(fn)||[])[1];
if(name){ret+=" "+name}ret+="(";ret=[ret,this.parse(fn,"functionArgs"),"){"].join("");
return join(ret,this.parse(fn,"functionCode"),"}")},array:array,nodelist:array,arguments:array,scobj:function(obj){return obj.toString()
},object:function(map){var ret=[];this.up();for(var key in map){ret.push(this.parse(key,"key")+": "+this.parse(map[key]))
}this.down();return join("{",ret,"}")},node:function(node){var open=this.HTML?"&lt;":"<",close=this.HTML?"&gt;":">";
var tag=node.nodeName.toLowerCase(),ret=open+tag;for(var a in this.DOMAttrs){var val=node[this.DOMAttrs[a]];
if(val){ret+=" "+a+"="+this.parse(val,"attribute")}}return ret+close+open+"/"+tag+close
},functionArgs:function(fn){var l=fn.length;if(!l){return""}var args=Array(l);while(l--){args[l]=String.fromCharCode(97+l)
}return" "+args.join(", ")+" "},key:quote,functionCode:"[code]",attribute:quote,string:quote,date:quote,regexp:literal,number:literal,"boolean":literal},DOMAttrs:{id:"id",name:"name","class":"className"},HTML:false,indentChar:"   ",multiline:true}
})();(function(){var reName,jsDump;function quote(str){return'"'+str.toString().replace(/"/g,'\\"')+'"'
}function literal(o){return o+""}function join(pre,arr,post){var s=jsDump.separator(),base=jsDump.indent(),inner=jsDump.indent(1);
if(arr.join){arr=arr.join(","+s+inner)}if(!arr){return pre+post}return[pre,inner+arr,base+post].join(s)
}function array(arr){var i=arr.length,ret=new Array(i);this.up();while(i--){ret[i]=this._parse(arr[i])
}this.down();return join("[",ret,"]")}reName=/^function (\w+)/;jsDump=CoreTest.jsDump={parse:function(obj,type){if(obj&&obj.toString){var toString=obj.toString;
if((toString!==Object.prototype.toString)&&(toString!==Array.toString)){return obj.toString()
}}if(obj&&obj.inspect){return obj.inspect()}this.seen=[];var ret=this._parse(obj,type);
this.seen=null;return ret},_parse:function(obj,type){var parser=this.parsers[type||this.typeOf(obj)];
type=typeof parser;if((parser===this.parsers.object)&&(this.seen.indexOf(obj)>=0)){return"(recursive)"
}this.seen.push(obj);return type=="function"?parser.call(this,obj):type=="string"?parser:this.parsers.error
},typeOf:function(obj){var type=typeof obj,f="function";if(obj&&(obj.isObject||obj.isClass)){return"scobj"
}return type!="object"&&type!=f?type:!obj?"null":obj.exec?"regexp":obj.getHours?"date":obj.scrollBy?"window":obj.nodeName=="#document"?"document":obj.nodeName?"node":obj.item?"nodelist":obj.callee?"arguments":obj.call||obj.constructor!=Array&&(obj+"").indexOf(f)!=-1?f:"length" in obj?"array":type
},separator:function(){return this.multiline?this.HTML?"<br />":"\n":this.HTML?"&nbsp;":" "
},indent:function(extra){if(!this.multiline){return""}var chr=this.indentChar;if(this.HTML){chr=chr.replace(/\t/g,"   ").replace(/ /g,"&nbsp;")
}return(new Array(this._depth_+(extra||0))).join(chr)},up:function(a){this._depth_+=a||1
},down:function(a){this._depth_-=a||1},setParser:function(name,parser){this.parsers[name]=parser
},quote:quote,literal:literal,join:join,_depth_:1,parsers:{window:"[Window]",document:"[Document]",error:"[ERROR]",unknown:"[Unknown]","null":"null","undefined":"undefined","function":function(fn){var ret="function",name="name" in fn?fn.name:(reName.exec(fn)||[])[1];
if(name){ret+=" "+name}ret+="(";ret=[ret,this._parse(fn,"functionArgs"),"){"].join("");
return join(ret,this._parse(fn,"functionCode"),"}")},array:array,nodelist:array,"arguments":array,scobj:function(obj){return obj.toString()
},object:function(map){var ret=[];this.up();for(var key in map){ret.push(this._parse(key,"key")+": "+this._parse(map[key]))
}this.down();return join("{",ret,"}")},node:function(node){var open=this.HTML?"&lt;":"<",close=this.HTML?"&gt;":">";
var tag=node.nodeName.toLowerCase(),ret=open+tag;for(var a in this.DOMAttrs){var val=node[this.DOMAttrs[a]];
if(val){ret+=" "+a+"="+this._parse(val,"attribute")}}return ret+close+open+"/"+tag+close
},functionArgs:function(fn){var l=fn.length;if(!l){return""}var args=new Array(l);
while(l--){args[l]=String.fromCharCode(97+l)}return" "+args.join(", ")+" "},key:quote,functionCode:"[code]",attribute:quote,string:quote,date:quote,regexp:literal,number:literal,"boolean":literal},DOMAttrs:{id:"id",name:"name","class":"className"},HTML:true,indentChar:"   ",multiline:true};
CoreTest.dump=function dump(obj,type){return CoreTest.jsDump.parse(obj,type)}})();
CoreTest.equiv=function(){var innerEquiv;var callers=[];function hoozit(o){if(typeof o==="string"){return"string"
}else{if(typeof o==="boolean"){return"boolean"}else{if(typeof o==="number"){if(isNaN(o)){return"nan"
}else{return"number"}}else{if(typeof o==="undefined"){return"undefined"}else{if(o===null){return"null"
}else{if(o instanceof Array){return"array"}else{if(o instanceof Date){return"date"
}else{if(o instanceof RegExp){return"regexp"}else{if(typeof o==="object"){return"object"
}else{if(o instanceof Function){return"function"}}}}}}}}}}}function bindCallbacks(o,callbacks,args){var prop=hoozit(o);
if(prop){if(hoozit(callbacks[prop])==="function"){return callbacks[prop].apply(callbacks,args)
}else{return callbacks[prop]}}}var callbacks=function(){function useStrictEquality(b,a){return a===b
...
if(b===a){return true}if(a.constructor!==b.constructor){return false}callers.push(a.constructor);
for(i in a){aProperties.push(i);if(!innerEquiv(a[i],b[i])){eq=false}}callers.pop();
for(i in b){bProperties.push(i)}return eq&&innerEquiv(aProperties.sort(),bProperties.sort())
}}}();innerEquiv=function(){var args=Array.prototype.slice.apply(arguments);if(args.length<2){return true
}return(function(a,b){if(a===b){return true}else{if(typeof a!==typeof b||a===null||b===null||typeof a==="undefined"||typeof b==="undefined"){return false
}else{if(b&&b.isEqual&&b.isEqual instanceof Function){return b.isEqual(a)}else{return bindCallbacks(a,callbacks,[b,a])
}}}})(args[0],args[1])&&arguments.callee.apply(this,args.splice(1,args.length-1))
};return innerEquiv}();sc_require("jquery");CoreTest.Plan={create:function(attrs){var len=arguments.length,ret=CoreTest.beget(this),idx;
for(idx=0;idx<len;idx++){CoreTest.mixin(ret,attrs)}ret.queue=ret.queue.slice();return ret
},queue:[],isRunning:false,synchronize:function synchronize(callback){this.queue.push(callback);
...
}return this},start:function(){var plan=this;setTimeout(function(){if(plan.timeout){clearTimeout(plan.timeout)
}plan.timeout=null;plan.isRunning=true;plan.process()},13);return this},stop:function(timeout){this.isRunning=false;
if(this.timeout){clearTimeout(this.timeout)}if(timeout){var plan=this;this.timeout=setTimeout(function(){plan.fail("Test timed out").start()
},timeout)}else{this.timeout=null}return this},pause:function(){if(this.isRunning){var del=this.delegate;
if(del&&del.planDidPause){del.planDidPause(this)}this.isRunning=false;this.start()
}return this},run:function(){this.isRunning=true;this.prepare();this.results={start:new Date().getTime(),finish:null,runtime:0,tests:0,total:0,passed:0,failed:0,errors:0,warnings:0,assertions:[]};
this.begin().synchronize(this.finish).process();return this},begin:function(){var del=this.delegate;
if(del&&del.planDidBegin){del.planDidBegin(this)}return this},finish:function(){var r=this.results,del=this.delegate;
r.finish=new Date().getTime();r.runtime=r.finish-r.start;if(del&&del.planDidFinish){del.planDidFinish(this,r)
}return this},module:function(desc,lifecycle){if(typeof SC!=="undefined"&&SC.filename){desc=SC.filename.replace(/^.+?\/current\/tests\//,"")+"\n"+desc
}this.currentModule=desc;if(!lifecycle){lifecycle={}}this.setup(lifecycle.setup).teardown(lifecycle.teardown);
return this},setup:function(func){this.currentSetup=func||CoreTest.K;return this},teardown:function teardown(func){this.currentTeardown=func||CoreTest.K;
return this},now:function(){return new Date().getTime()},test:function test(desc,func){if(!this.enabled(this.currentModule,desc)){return this
}var working={module:this.currentModule,test:desc,expected:0,assertions:[]};var msg;
var name=desc;if(this.currentModule){name=this.currentModule+" module: "+name}var setup=this.currentSetup||CoreTest.K;
var teardown=this.currentTeardown||CoreTest.K;this.synchronize(function(){var mainEl=document.getElementById("main");
this.fixture=mainEl?mainEl.innerHTML:"";mainEl=null;this.working=working;try{working.total_begin=working.setup_begin=this.now();
setup.call(this);working.setup_end=this.now()}catch(e){msg=(e&&e.toString)?e.toString():"(unknown error)";
this.error("Setup exception on "+name+": "+msg)}});this.synchronize(function(){if(!func){this.warn("Test not yet implemented: "+name)
}else{try{if(CoreTest.trace){console.log("run: "+name)}this.working.test_begin=this.now();
func.call(this);this.working.test_end=this.now()}catch(e){msg=(e&&e.toString)?e.toString():"(unknown error)";
this.error("Died on test #"+(this.working.assertions.length+1)+": "+msg)}}});this.synchronize(function(){try{this.working.teardown_begin=this.now();
teardown.call(this);this.working.teardown_end=this.now()}catch(e){msg=(e&&e.toString)?e.toString():"(unknown error)";
this.error("Teardown exception on "+name+": "+msg)}});this.synchronize(function(){if(this.reset){try{this.working.reset_begin=this.now();
this.reset();this.working.total_end=this.working.reset_end=this.now()}catch(ex){msg=(ex&&ex.toString)?ex.toString():"(unknown error)";
this.error("Reset exception on "+name+": "+msg)}}var w=this.working,exp=w.expected,len=w.assertions.length;
if(exp&&exp!==len){this.fail("Expected "+exp+" assertions, but "+len+" were run")
}this.working=null;this.record(w.module,w.test,w.assertions,w);if(!this.pauseTime){this.pauseTime=new Date().getTime()
}else{var now=new Date().getTime();if((now-this.pauseTime)>250){this.pause();this.pauseTime=now
}}})},clearHtmlbody:function(){var body=Q$("body")[0];var begin=Q$("body #htmlbody-begin")[0];
if(!begin){begin=Q$('<div id="htmlbody-begin"></div>')[0];body.appendChild(begin)
}else{while(begin.nextSibling){body.removeChild(begin.nextSibling)}}begin=null},htmlbody:function htmlbody(string){var html=Q$(string);
var body=Q$("body")[0];this.clearHtmlbody();html.each(function(){body.appendChild(this)
})},record:function(module,test,assertions,timings){var r=this.results,len=assertions.length,del=this.delegate,idx,cur;
r.tests++;for(idx=0;idx<len;idx++){cur=assertions[idx];cur.module=module;cur.test=test;
r.total++;r[cur.result]++;r.assertions.push(cur)}if(del&&del.planDidRecord){del.planDidRecord(this,module,test,assertions,timings)
}},reset:function(){if(this.fixture){var mainEl=document.getElementById("main");if(mainEl){mainEl.innerHTML=this.fixture
}mainEl=null}return this},enabled:function(moduleName,testName){return true},pass:function(msg){var w=this.working;
if(!w){throw"pass("+msg+") called outside of a working test"}w.assertions.push({message:msg,result:CoreTest.OK});
return this},fail:function(msg){var w=this.working;if(!w){throw"fail("+msg+") called outside of a working test"
}w.assertions.push({message:msg,result:CoreTest.FAIL});return this},warn:function(msg){var w=this.working;
if(!w){throw"warn("+msg+") called outside of a working test"}w.assertions.push({message:msg,result:CoreTest.WARN});
return this},error:function(msg,e){var w=this.working;if(!w){throw"error("+msg+") called outside of a working test"
}if(e&&typeof console!="undefined"&&console.error&&console.warn){console.error(msg);
console.error(e)}w.assertions.push({message:msg,result:CoreTest.ERROR});return this
},fn:{ok:function ok(pass,actual,expected,msg){if(msg===undefined){msg=actual;if(!msg){msg=pass?"OK":"failed"
}}else{if(!msg){msg=pass?"OK":"failed"}if(pass){msg=msg+": "+CoreTest.dump(expected)
}else{msg=msg+", expected: "+CoreTest.dump(expected)+" result: "+CoreTest.dump(actual)
}}return !!pass?this.pass(msg):this.fail(msg)},equals:function equals(actual,expected,msg){if(msg===undefined){msg=null
}return this.ok(actual==expected,actual,expected,msg)},should_throw:function should_throw(callback,expected,msg){var actual=false;
try{callback()}catch(e){actual=(typeof expected==="string")?e.message:e}if(expected===false){ok(actual===false,CoreTest.fmt("%@ expected no exception, actual %@",msg,actual))
}else{if(expected===Error||expected===null||expected===true){ok(!!actual,CoreTest.fmt("%@ expected exception, actual %@",msg,actual))
}else{equals(actual,expected,msg)}}},expect:function expect(asserts){this.working.expected=asserts
},same:function(actual,expected,msg){if(msg===undefined){msg=null}return this.ok(CoreTest.equiv(actual,expected),actual,expected,msg)
},stop:function(timeout){return this.stop(timeout)},start:function(){return this.start()
},reset:function(){return this.reset()}},prepare:function(){var fn=this.fn,plan=this,key,func;
for(key in fn){if(!fn.hasOwnProperty(key)){continue}func=fn[key];if(typeof func!=="function"){continue
}window[key]=this._bind(func);if(!plan[key]){plan[key]=func}}return this},_bind:function(func){var plan=this;
return function(){return func.apply(plan,arguments)}}};CoreTest.defaultPlan=function defaultPlan(){var plan=CoreTest.plan;
if(!plan){CoreTest.runner=CoreTest.Runner.create();plan=CoreTest.plan=CoreTest.runner.plan
}return plan};window.module=function(desc,l){CoreTest.defaultPlan().module(desc,l)
};window.test=function(desc,func){CoreTest.defaultPlan().test(desc,func)};window.clearHtmlbody=function(){CoreTest.defaultPlan().clearHtmlbody()
};window.htmlbody=function(string){CoreTest.defaultPlan().htmlbody(string)};sc_require("jquery");
sc_require("system/plan");CoreTest.Runner={plan:null,errors:null,create:function(){var len=arguments.length,ret=CoreTest.beget(this),idx;
for(idx=0;idx<len;idx++){CoreTest.mixin(ret,arguments[len])}if(!ret.plan){ret.plan=CoreTest.Plan.create({delegate:ret})
}window.resizeTo(1400,800);Q$(window).load(function(){ret.begin()});return ret},begin:function(){var plan=CoreTest.plan;
plan.delegate=this;plan.run()},planDidBegin:function(plan){this.report=Q$(['<div class="core-test">','<div class="useragent">UserAgent</div>','<div class="testresult">','<label class="hide-passed">','<input type="checkbox" checked="checked" /> Hide passed tests',"</label>",'<span class="status">Running...</span>',"</div>",'<div class="detail">',"<table>","<thead><tr>",'<th class="desc">Test</th><th>Result</th>',"</tr></thead>","<tbody><tr></tr></tbody>","</table>","</div>","</div>"].join(""));
this.report.find(".useragent").html(navigator.userAgent);this.logq=this.report.find("tbody");
this.testCount=0;var runner=this;this.checkbox=this.report.find(".hide-passed input");
this.checkbox.change(function(){runner.hidePassedTestsDidChange()});Q$("body").append(this.report)
},hidePassedTestsDidChange:function(){var checked=!!this.checkbox.val();if(checked){this.logq.addClass("hide-clean")
}else{this.logq.removeClass("hide-clean")}},planDidFinish:function(plan,r){this.flush();
var result=this.report.find(".testresult .status");var str=CoreTest.fmt('<span>Completed %@ tests in %@ msec. </span><span class="total">%@</span> total assertions: ',r.tests,r.runtime,r.total);
if(r.passed>0){str+=CoreTest.fmt('&nbsp;<span class="passed">%@ passed</span>',r.passed)
}if(r.failed>0){str+=CoreTest.fmt('&nbsp;<span class="failed">%@ failed</span>',r.failed)
}if(r.errors>0){str+=CoreTest.fmt('&nbsp;<span class="errors">%@ error%@</span>',r.errors,(r.errors!==1?"s":""))
}if(r.warnings>0){str+=CoreTest.fmt('&nbsp;<span class="warnings">%@ warning%@</span>',r.warnings,(r.warnings!==1?"s":""))
}if(this.errors){this.errors.push("</tr></tbody></table>")}if((r.failed+r.errors+r.warnings)>0){this.hidePassedTestsDidChange()
...
}result.html(str);if(this.errors){CoreTest.errors=this.errors.join("")}if(SC&&SC.Event&&SC.Event.unload){try{SC.Event.unload()
}catch(e){}}},planDidRecord:function(plan,module,test,assertions,timings){var name=test,s={passed:0,failed:0,errors:0,warnings:0},len=assertions.length,clean="",idx,cur,q;
for(idx=0;idx<len;idx++){s[assertions[idx].result]++}if((s.failed+s.errors+s.warnings)===0){clean="clean"
}if(module){name=module.replace(/\n/g,"<br />")+" module: "+test}name=CoreTest.fmt("%@ - %@msec",name,timings.total_end-timings.total_begin);
var logstr=this.logstr;var errors=this.errors;if(!logstr){logstr=this.logstr=[]}if(!this.errors){this.errors=['<style type="text/css">* {font: 12px arial;}.passed { background-color: #80D175; color: white;}.failed { background-color: #ea4d4; color: black; }.errors { background-color: red; color: black; }.warnings { background-color: #E49723; color: black;}.desc { text-align: left;}</style><table style="border:1px solid"><thead><tr><th class="desc">'+navigator.userAgent+"</th><th>Result</th></tr></thead><tbody><tr>"]
...
if(s.failed>0||s.errors>0){this.errors.push(CoreTest.fmt('<tr class="test %@"><th style="background:grey; color:white" class="desc" colspan="2">%@ (<span class="passed">%@</span>, <span class="failed">%@</span>, <span class="errors">%@</span>, <span class="warnings">%@</span>)</th></tr>',clean,name,s.passed,s.failed,s.errors,s.warnings))
}len=assertions.length;for(idx=0;idx<len;idx++){cur=assertions[idx];clean=cur.result===CoreTest.OK?"clean":"dirty";
...
if(clean=="dirty"){this.errors.push(CoreTest.fmt('<tr class="%@"><td class="desc">%@</td><td class="action %@">%@</td></tr>',clean,cur.message,cur.result,(cur.result||"").toUpperCase()))
}}this.testCount++;this.resultStr=CoreTest.fmt("Running – Completed %@ tests so far.",this.testCount)
},planDidPause:function(plan){if(navigator.userAgent.indexOf("MSIE")==-1){this.flush()
}},flush:function(){var logstr=this.logstr,resultStr=this.resultStr,result=this.report.find(".testresult .status");
if(logstr){this.logq.append(this.logstr.join(""))}if(resultStr){result.html(resultStr)
}this.resultStr=this.logstr=null}};CoreTest.Suite={create:function(desc,attrs){var len=arguments.length,ret=CoreTest.beget(this),idx;
for(idx=1;idx<len;idx++){CoreTest.mixin(ret,arguments[idx])}if(desc){ret.basedesc=desc
}ret.definitions=ret.definitions.slice();return ret},generate:function(desc,attrs){var len=arguments.length,ret=CoreTest.beget(this),idx,defs;
for(idx=1;idx<len;idx++){CoreTest.mixin(ret,arguments[idx])}ret.subdesc=desc;defs=ret.definitions;
len=defs.length;for(idx=0;idx<len;idx++){defs[idx].call(ret,ret)}return ret},define:function(func){this.definitions.push(func);
return this},definitions:[],desc:function(str){return this.basedesc.fmt(this.subdesc,str)
},basedesc:"%@ > %@",setup:function(){this.object=this.newObject()},teardown:function(){if(this.object){this.destroyObject(this.object)
}this.object=null},newObject:function(){return null},destroyObject:function(obj){},module:function(desc){var T=this;
module(T.desc(desc),{setup:function(){T.setup()},teardown:function(){T.teardown()
}})}};